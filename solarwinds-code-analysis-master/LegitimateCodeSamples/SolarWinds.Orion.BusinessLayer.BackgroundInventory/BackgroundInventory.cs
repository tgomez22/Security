...
namespace SolarWinds.Orion.Core.BusinessLayer.BackgroundInventory
{
  public class BackgroundInventory : IDisposable
  {
    ...
    public virtual bool IsScheduledTaskCanceled() => this.scheduler.IsTaskCanceled;
    ...
    public BackgroundInventory(int parallelTasksCount, Dictionary<string, object> plugins)
    {
      ...
      this.snmpGlobals.HsrpEnabled = (bool) SettingsDAL.GetCurrent<bool>("SWNetPerfMon-Settings-SNMP HSRPEnabled", (M0) 1);
      this.wmiGlobals = new WmiGlobalSettings();
      this.wmiGlobals.UserImpersonationLevel = (ImpersonationLevel) SettingsDAL.GetCurrent<ImpersonationLevel>("SWNetPerfMon-Settings-Wmi UserImpersonationLevel", (M0) 0);
      this.wmiGlobals.ConnectionRationMode = (WmiConnectionRationMode) SettingsDAL.GetCurrent<WmiConnectionRationMode>("SWNetPerfMon-Settings-Wmi ConnectionRationMode", (M0) 1);
      this.wmiGlobals.MaxRationedConnections = SettingsDAL.GetCurrentInt("SWNetPerfMon-Settings-Wmi MaxRationedConnections", 0);
      this.wmiGlobals.KillProcessExcessiveError = (bool) SettingsDAL.GetCurrent<bool>("SWNetPerfMon-Settings-Wmi KillProcessExcessiveError", (M0) 1);
    ...
    private void scheduler_TaskProcessingFinished(object sender, EventArgs e) => SolarWinds.Orion.Core.BusinessLayer.BackgroundInventory.BackgroundInventory.log.Info((object) "Background Inventorying Finished");
    ...
    public virtual Node GetNode(int nodeId) => NodeBLDAL.GetNode(nodeId);

    public virtual Credential GetCredentialsForNode(Node node) => CredentialHelper.ParseCredentialsFromNode(node);
    ...
    public void DoInventory(SolarWinds.Orion.Core.BusinessLayer.BackgroundInventory.BackgroundInventory.InventoryTask task)
    {
      ...
      else
      {
        ...
        if (SolarWinds.Orion.Core.BusinessLayer.BackgroundInventory.BackgroundInventory.log.IsInfoEnabled)
        ...
        foreach (string key in strArray)
        {
          ...
          {
            failedTasks.Add(key);
            if (SolarWinds.Orion.Core.BusinessLayer.BackgroundInventory.BackgroundInventory.log.IsErrorEnabled)
            ...
            else
            {
              ...
              else
              {
                ...
                else
                {
                  ...
                  {
                    if (SolarWinds.Orion.Core.BusinessLayer.BackgroundInventory.BackgroundInventory.log.IsWarnEnabled)
                      SolarWinds.Orion.Core.BusinessLayer.BackgroundInventory.BackgroundInventory.log.WarnFormat("Inventory '{0}' on Node {1} failed with code {2}", (object) key, (object) task, (object) result.Error.ErrorCode);
                    if (result.Error.ErrorCode != 31002U)
                    {
                  ...
                  else if (SolarWinds.Orion.Core.BusinessLayer.BackgroundInventory.BackgroundInventory.log.IsWarnEnabled)
        ...
        if ((string.IsNullOrEmpty(settingsForTask) || !settingsForTask.Equals(task.Settings, StringComparison.OrdinalIgnoreCase)) && SolarWinds.Orion.Core.BusinessLayer.BackgroundInventory.BackgroundInventory.log.IsInfoEnabled)
        {
        ...
        else
        {
          ...
          else if (SolarWinds.Orion.Core.BusinessLayer.BackgroundInventory.BackgroundInventory.log.IsInfoEnabled)
    ...
    ~BackgroundInventory() => this.Dispose(false);
    ...
    public class InventoryTask
    {
      ...
      public override string ToString() => string.Format("NodeID = {0}, NodeSettingID = {1}, Settings = {2}, InventorySettingName = {3}, ObjectID = {4}, ObjectType = {5}", (object) this.NodeID, (object) this.ObjectSettingID, (object) this.Settings, (object) this.InventorySettingName, (object) this.ObjectID, (object) this.ObjectType);
