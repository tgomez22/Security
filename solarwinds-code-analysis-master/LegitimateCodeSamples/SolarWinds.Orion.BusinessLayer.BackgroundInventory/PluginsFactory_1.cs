...
namespace SolarWinds.Orion.Core.BusinessLayer.BackgroundInventory
{
  internal sealed class PluginsFactory<T>
  {
    ...
    public PluginsFactory()
      : this(OrionConfiguration.InstallPath, "*.Pollers.dll", "*.Collector.dll", "*.Plugin.dll")
    {
    }
    ...
    private void Process(IEnumerable<string> files)
    {
      ...
      Dictionary<string, FileInfo> dictionary = new DirectoryXmlFilePluginsProvider(OrionConfiguration.InstallPath).ReadAreas().Where<PluginsArea>((Func<PluginsArea, bool>) (n => n.Name.Equals("BackgroundInventory"))).SelectMany<PluginsArea, Plugin>((Func<PluginsArea, IEnumerable<Plugin>>) (area => (IEnumerable<Plugin>) area.Plugins)).Select<Plugin, FileInfo>((Func<Plugin, FileInfo>) (plugin => new FileInfo(Path.Combine(basePath, plugin.AssemblyPath)))).Where<FileInfo>((Func<FileInfo, bool>) (info => info.Exists)).ToDictionary<FileInfo, string>((Func<FileInfo, string>) (info => info.Name), (IEqualityComparer<string>) StringComparer.OrdinalIgnoreCase);
    ...
    private sealed class AssemblyVersionInfo : 
      IComparable<PluginsFactory<T>.AssemblyVersionInfo>,
      IEquatable<PluginsFactory<T>.AssemblyVersionInfo>
    {
      ...
      internal static PluginsFactory<T>.AssemblyVersionInfo Create(string fileName) => new PluginsFactory<T>.AssemblyVersionInfo(fileName);
      ...
      public bool Equals(PluginsFactory<T>.AssemblyVersionInfo other) => StringComparer.OrdinalIgnoreCase.Equals(this.AssemblyName.Name, other.AssemblyName.Name);

      public override string ToString() => this.AssemblyName.ToString() + " @ " + (object) this.FileInfo + (object) ':' + (object) this.FileVersion;
    }
  }
}
